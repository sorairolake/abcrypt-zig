...
test "get Argon2 parameters" {
    {
        const params = try root.Params.init(@embedFile("data/v1/argon2d/v0x10/data.txt.abcrypt"));
    ...
    {
        const params = try root.Params.init(@embedFile("data/v1/argon2d/v0x13/data.txt.abcrypt"));
        try testing.expectEqual(19456, params.memory_cost);
        try testing.expectEqual(2, params.time_cost);
        try testing.expectEqual(1, params.parallelism);
    }
    {
        const params = try root.Params.init(@embedFile("data/v1/argon2i/v0x10/data.txt.abcrypt"));
        try testing.expectEqual(12288, params.memory_cost);
        try testing.expectEqual(3, params.time_cost);
        try testing.expectEqual(1, params.parallelism);
    }
    {
        const params = try root.Params.init(@embedFile("data/v1/argon2i/v0x13/data.txt.abcrypt"));
        try testing.expectEqual(9216, params.memory_cost);
        try testing.expectEqual(4, params.time_cost);
        try testing.expectEqual(1, params.parallelism);
    }
    {
        const params = try root.Params.init(@embedFile("data/v1/argon2id/v0x10/data.txt.abcrypt"));
